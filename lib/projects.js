...
export async function replaceItemAssignees(client, itemId, projectId, assigneeIds = []) {
  ...
  if (assigneeIds.length) {
    const valid = await client.query("SELECT COUNT(*)::int AS count FROM project_members WHERE project_id = $1 AND id = ANY($2::uuid[])", [projectId, assigneeIds]);
...
export const projectSelect = `project.id, project.title, project.description,
  ...
  project.status, project.progress, project.created_at AS "createdAt", project.updated_at AS "updatedAt"`;
