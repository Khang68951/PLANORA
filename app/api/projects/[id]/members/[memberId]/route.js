...
export async function PATCH(request, context) {
  try {
    ...
    const current = (await query("SELECT name, role FROM project_members WHERE id = $1 AND project_id = $2 AND deleted_at IS NULL", [memberId, id])).rows[0];
    ...
    const result = await query(`UPDATE project_members SET name = $1, role = $2, updated_at = NOW() WHERE id = $3 AND project_id = $4 AND deleted_at IS NULL
...
export async function DELETE(_request, context) {
  try {
    const { id, memberId } = await context.params;
    if (!isUuid(id) || !isUuid(memberId)) return NextResponse.json({ error: "Invalid member." }, { status: 400 });
    const result = await query("DELETE FROM project_members WHERE id = $1 AND project_id = $2 AND deleted_at IS NULL RETURNING id", [memberId, id]);
