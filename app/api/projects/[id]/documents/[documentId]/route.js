...
export async function PATCH(request, context) {
  try {
    ...
    const current = (await query(`SELECT title, content_html AS "contentHtml", updated_at AS "updatedAt" FROM project_documents WHERE id = $1 AND project_id = $2`, [documentId, id])).rows[0];
    ...
    const result = await query(`UPDATE project_documents SET title = $1, content_html = $2, updated_at = NOW() WHERE id = $3 AND project_id = $4
...
export async function DELETE(_request, context) {
  try {
    const { id, documentId } = await context.params;
    if (!isUuid(id) || !isUuid(documentId)) return NextResponse.json({ error: "Invalid document." }, { status: 400 });
    const result = await query("DELETE FROM project_documents WHERE id = $1 AND project_id = $2 RETURNING id", [documentId, id]);
