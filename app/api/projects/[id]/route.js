...
export async function GET(_request, context) {
  try {
    ...
    const [project, members, documents, files, items, messages, tools] = await Promise.all([
      query(`SELECT ${projectSelect} ${projectFrom} WHERE project.id = $1 AND project.deleted_at IS NULL`, [id]),
      query(`SELECT id, name, role, created_at AS "createdAt", updated_at AS "updatedAt" FROM project_members WHERE project_id = $1 ORDER BY lower(name)`, [id]),
      query(`SELECT id, title, content_html AS "contentHtml", created_at AS "createdAt", updated_at AS "updatedAt" FROM project_documents WHERE project_id = $1 ORDER BY created_at`, [id]),
      query(`SELECT id, original_name AS name, mime_type AS "mimeType", size_bytes::int AS "sizeBytes", created_at AS "createdAt" FROM project_files WHERE project_id = $1 ORDER BY created_at DESC`, [id]),
