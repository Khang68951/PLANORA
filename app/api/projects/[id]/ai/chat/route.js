...
async function projectContext(id, scopes) {
  const includes = (scope) => scopes.includes(scope);
  const [project, members, items, documents, files, history, settings] = await Promise.all([
    ...
    includes("members") || includes("work") ? query("SELECT id, name, role FROM project_members WHERE project_id = $1 AND deleted_at IS NULL ORDER BY name", [id]) : Promise.resolve({ rows: [] }),
    includes("work") ? query(`SELECT item.id, item.title, item.description, item.kind, item.start_at, item.end_at, item.due_at, item.status, item.priority, item.updated_at AS "updatedAt",
      ...
      WHERE item.project_id = $1 AND item.deleted_at IS NULL GROUP BY item.id ORDER BY COALESCE(item.start_at, item.due_at)`, [id]) : Promise.resolve({ rows: [] }),
    includes("documents") ? query(`SELECT id, title, content_html AS "contentHtml", updated_at AS "updatedAt"
      FROM project_documents WHERE project_id = $1 AND deleted_at IS NULL ORDER BY updated_at DESC`, [id]) : Promise.resolve({ rows: [] }),
    includes("files") ? query("SELECT id, original_name AS name, stored_name, mime_type AS \"mimeType\", size_bytes::int AS \"sizeBytes\" FROM project_files WHERE project_id = $1 AND deleted_at IS NULL ORDER BY created_at DESC", [id]) : Promise.resolve({ rows: [] }),
  ...
  for (const file of includes("files") ? files.rows : []) {
...
export async function POST(request, context) {
  try {
    ...
    const contextData = await projectContext(id, workflow.scopes);
    if (!contextData) return NextResponse.json({ error: "Project not found." }, { status: 404 });
    const runResult = await query(`INSERT INTO project_ai_runs (project_id,request_message,intent,scopes,status)
      VALUES ($1,$2,$3,$4,'planning') RETURNING id,project_id AS "projectId",request_message AS "requestMessage",response_message AS "responseMessage",intent,scopes,status,created_at AS "createdAt",updated_at AS "updatedAt"`,
      [id, body.message.trim(), workflow.intent, JSON.stringify(workflow.scopes)]);
    const aiRun = runResult.rows[0];
    ...
    let providerStream;
    try {
      providerStream = await openProjectChatStream({ ...contextData.config, messages, signal: request.signal });
    } catch (error) {
      await query("UPDATE project_ai_runs SET status='failed',updated_at=NOW() WHERE id=$1", [aiRun.id]).catch(() => {});
      throw error;
    }
      ...
      start(controller) {
        ...
        try {
          ...
          const expectsCommands = workflow.allowedCommands.some((command) => command.safety !== "read")
            && /\b(?:create|write|add|insert|remove|delete|edit|update|rename|assign|schedule|set|make|move|trash)\b/i.test(body.message);
          let plannerParsed = { commands: [], clarificationQuestion: "", parseFailed: false };
          let plannerError = "";
          if (expectsCommands) {
            try {
              const plannerRaw = await collectProjectAIResponse({ ...contextData.config, messages: plannerMessages, maxTokens: 6000, signal: request.signal });
          ...
          for (const requested of needsClarification ? [] : requestedCommands) {
            ...
            const inserted = await query(`INSERT INTO project_ai_commands (project_id,run_id,name,safety,mode,arguments,summary,request_message,status)
              VALUES ($1,$2,$3,$4,$5,$6,$7,NULL,$8) RETURNING id,run_id AS "runId",name,safety,mode,arguments,summary,status,result,error,created_at AS "createdAt",decided_at AS "decidedAt"`,
              [id, aiRun.id, requested.name, requested.safety, contextData.project.aiCommandMode, requested.arguments, requested.summary, needsApproval ? "pending" : "running"]);
            ...
            if (!needsApproval) {
              try {
                ...
                const applied = await query(`UPDATE project_ai_commands SET status='applied',result=$1,decided_at=NOW() WHERE id=$2
                  RETURNING id,run_id AS "runId",name,safety,mode,arguments,summary,status,result,error,created_at AS "createdAt",decided_at AS "decidedAt"`, [result, command.id]);
                ...
                const failed = await query(`UPDATE project_ai_commands SET status='failed',error=$1,decided_at=NOW() WHERE id=$2
                  RETURNING id,run_id AS "runId",name,safety,mode,arguments,summary,status,result,error,created_at AS "createdAt",decided_at AS "decidedAt"`, [commandError.message, command.id]);
          ...
          const failedCommands = commandModels.some((command) => command.status === "failed");
          const runStatus = needsClarification ? "complete" : awaitingReview ? "pending" : failedCommands ? "partial" : "complete";
          ...
          try {
            ...
            const updatedRun = (await client.query(`UPDATE project_ai_runs SET response_message=$1,status=$2,updated_at=NOW() WHERE id=$3
              RETURNING id,project_id AS "projectId",request_message AS "requestMessage",response_message AS "responseMessage",intent,scopes,status,created_at AS "createdAt",updated_at AS "updatedAt"`, [responseMessage.slice(0, 20_000), runStatus, aiRun.id])).rows[0];
            ...
            controller.enqueue(streamEvent("done", { messages: savedMessages, commands: commandModels, run: toAIRunModel(updatedRun, commandModels), workflow: { intent: workflow.intent, scopes: workflow.scopes, stage: needsClarification ? "complete" : awaitingReview ? "review" : "complete" }, mode: contextData.project.aiCommandMode, provider: contextData.config.provider, model: contextData.config.model }));
          } catch (error) { await client.query("ROLLBACK"); throw error; } finally { client.release(); }
        } catch (error) {
          console.error("Stream project AI chat", error);
          await query("UPDATE project_ai_commands SET status='failed',error=$1,decided_at=NOW() WHERE run_id=$2 AND status IN ('pending','running')", [`Workflow stopped before review: ${error.message}`, aiRun.id]).catch(() => {});
          await query("UPDATE project_ai_runs SET status='failed',updated_at=NOW() WHERE id=$1", [aiRun.id]).catch(() => {});
